...
class integrationTicketService
{
    ...
    public function createTicket($data): JsonResponse
    {
        ...
        if ( $data->accountId == '1dd5bd55-d141-11ec-0a80-055600047495') $kassClient = new testKassClient($this->data->setting_main->kassa_token);
        else $kassClient = new integrationKassClient($this->data->setting_main->kassa_token);
        ...
        try {
            $postTicket = $kassClient->postCheck($Body, $this->data->setting_main->serial_number);
        ...
        try {
            ...
            if ($this->data->setting_document->paymentDocument != null ){
                $this->createPaymentDocument($this->data->setting_document, $entity_type, $put, $Body['Payments']);
            }
        } catch (BadResponseException $e){
            return response()->json([
    ...
    private function createPaymentDocument( $Setting, string $entity_type, mixed $OldBody, mixed $payments): void
    {
